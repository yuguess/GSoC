...
{
	template <typename T>
	bool mergeElement(T *pData, int rowCount, int colCount, DataAccessor pSrcAcc, int i)
	{
		char fileName[100];
...
bool DataMerge::showGui()
{
	...
	mpRaster1 = RasterUtilities::createRasterElement("DataMergeCube", mNumRows,
      mNumCols, mNumBands, FLT8BYTES, BSQ, true, NULL);
	if (mpRaster1 == NULL)
	{
		QMessageBox::critical(NULL, "Spectral Data Merge", "No RasterElement input found!", "OK");
		pStep->finalize(Message::Failure, "No RasterElement input found!");
